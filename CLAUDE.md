...
- **NEXT (user's plan)**: (1) consolidate the imperative `az` provisioning into **Bicep** (currently only OpenAI is Bicep; ACR/identity/Container Apps/SQL were done via `az` — documented in `infra/container-apps.md`), then (2) **CI/CD** (GitHub Actions + OIDC → `az acr build` + `containerapp update` on push). Optional later: scheduled Container Apps Job, App Insights/OpenTelemetry. Stretch: Bing-grounded cross-shipment intelligence, dashboard.
