...
- **NEXT (user's plan)**: **CI/CD** — GitHub Actions + OIDC (federated credential, no stored secret) → `az acr build` + `az containerapp update` on push to main. Optional later: scheduled Container Apps Job, App Insights/OpenTelemetry. Stretch: Bing-grounded cross-shipment intelligence, dashboard.
