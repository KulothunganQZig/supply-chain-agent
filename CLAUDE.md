...
- **NEXT**: Azure SQL migration (swap SQLite → Azure SQL; add `msodbcsql18`+`unixodbc` to the Dockerfile, AAD DB user for the app identity). Then optional: scheduled Container Apps Job (periodic runs), App Insights/OpenTelemetry, CI/CD (GitHub Actions + OIDC). Stretch: Bing-grounded cross-shipment intelligence, dashboard.
